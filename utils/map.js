...
class Chunk {
  ...
  generate() {
    if (Math.abs(this.cx) > 25 || Math.abs(this.cy) > 25) {
      for (let x = 0; x < CHUNKSIZE; x++) {
        for (let y = 0; y < CHUNKSIZE; y++) {
          ...
          if (x % 2 == 0 && y % 2 == 0) {
            let temp = new Placeable(
              'BearTrap',
              (x + this.cx * CHUNKSIZE) * TILESIZE,
              (y + this.cy * CHUNKSIZE) * TILESIZE,
              0,
              68,
              48,
              1,
    ...
    if (rand < 0.2) {
      ...
      let m1 = new Placeable(
        'Mushroom',
        (structX + this.cx * CHUNKSIZE) * TILESIZE,
        (structY + this.cy * CHUNKSIZE) * TILESIZE,
        0,
        120,
        120,
        0,
      ...
      let m2 = new Placeable(
        'Mushroom',
        (structX + randX1 + this.cx * CHUNKSIZE) * TILESIZE,
        (structY + randY1 + this.cy * CHUNKSIZE) * TILESIZE,
        0,
        120,
        120,
        0,
      ...
      let m3 = new Placeable(
        'Mushroom',
        (structX + randX2 + this.cx * CHUNKSIZE) * TILESIZE,
        (structY + randY2 + this.cy * CHUNKSIZE) * TILESIZE,
        0,
        120,
        120,
        0,
    ...
    if (rand < 0.3 && !(this.cx == 0 && this.cy == 0)) {
      //add turrets
      let structX = Math.floor(Math.random() * CHUNKSIZE);
      let structY = Math.floor(Math.random() * CHUNKSIZE);
      ...
      let temp = new Placeable(
        'Chest',
        (sructX + this.cx * CHUNKSIZE) * TILESIZE,
        (structY + this.cy * CHUNKSIZE) * TILESIZE,
        0,
        120,
        120,
      ...
      if (Math.random() < 0.7)
        temp.invBlock.items['Bomb'] = { amount: Math.floor(Math.random() * 5) + 1 };
      if (Math.random() < 0.5)
        temp.invBlock.items['Tech'] = { amount: Math.floor(Math.random() * 3) + 1 };
      if (Math.random() < 0.2) temp.invBlock.items['Fire Staff'] = { amount: 1 };
      if (Math.random() < 0.2) temp.invBlock.items['Laser Gun'] = { amount: 1 };
      if (Math.random() < 0.2)
        temp.invBlock.items["Philosopher's Stone"] = { amount: Math.floor(Math.random() * 5) + 1 };
      if (Math.random() < 0.4)
          ...
          if (x == 0 || x == 8 || y == 0 || y == 8) {
            if (x == 4) {
              if (Math.random() < 0.9) {
                temp = new Placeable(
                  'Floor',
                  ((x + 2.1) * 3.98 + this.cx * CHUNKSIZE) * TILESIZE,
                  ((y + 2.1) * 3.98 + this.cy * CHUNKSIZE) * TILESIZE,
                  0,
                  128,
                  128,
                  0,
            ...
            temp = new Placeable(
              'Chest',
              ((x + 2.1) * 3.98 + this.cx * CHUNKSIZE) * TILESIZE,
              ((y + 1.7) * 3.98 + this.cy * CHUNKSIZE) * TILESIZE,
              0,
              128,
              128,
              0,
            ...
            if (Math.random() < 0.7)
              temp.invBlock.items['Bomb'] = { amount: Math.floor(Math.random() * 8) + 1 };
            if (Math.random() < 0.5)
              temp.invBlock.items['Tech'] = { amount: Math.floor(Math.random() * 6) + 1 };
            if (Math.random() < 0.2) temp.invBlock.items['Fire Staff'] = { amount: 1 };
            if (Math.random() < 0.2) temp.invBlock.items['Laser Gun'] = { amount: 1 };
            if (Math.random() < 0.2)
              temp.invBlock.items["Philosopher's Stone"] = {
                amount: Math.floor(Math.random() * 9) + 1,
              };
            if (Math.random() < 0.4)
              temp.invBlock.items['Black Gem'] = { amount: Math.floor(Math.random() * 10) + 1 };
            this.objects.push(temp);
            temp = new Placeable(
              'Chest',
              ((x + 2.1) * 3.98 + this.cx * CHUNKSIZE) * TILESIZE,
              ((y + 2.4) * 3.98 + this.cy * CHUNKSIZE) * TILESIZE,
              0,
              128,
              128,
              0,
